...
<div class="relative grow">
	<textarea
		class="scrollable focus:border-secondary-600 dark:focus:border-secondary-300 peer w-full rounded border-2 border-stone-400 bg-stone-50 px-3.5 pt-2 text-stone-900 
	...
	<label
		...
		class="peer-focus:text-secondary-700 dark:peer-focus:text-secondary-300 pointer-events-none absolute left-2 -top-2.5 rounded bg-stone-50 px-2 text-sm text-stone-600 transition-all peer-placeholder-shown:top-2 peer-placeholder-shown:text-base peer-placeholder-shown:text-stone-500 peer-focus:-top-2.5 peer-focus:text-sm dark:bg-stone-700 dark:text-stone-300 dark:peer-placeholder-shown:text-stone-300"
		>{$form.controls[formControlName].label}</label
	>
	{#if $form.controls[formControlName].hint && !$form.controls[formControlName].errors}
		<div class="absolute -bottom-4 left-2 text-xs text-stone-500">
